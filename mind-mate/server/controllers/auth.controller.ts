...
export async function login(
    ...
    try{
        ...
        if(loginUser){
            const accessToken = jwt.sign({userid: loginUser.userid, nickname: loginUser.nickname, password: loginUser.password}, JWT_SECRET, {expiresIn: '15m'});
            return res.json({
                msg: '로그인 성공',
...
export async function loginAgain(
    ...
    try{ 
        ...
        const accessToken = jwt.sign({userid: userid, nickname: nickname, password: password}, JWT_SECRET, {expiresIn: '15m'});
        return res.json({
